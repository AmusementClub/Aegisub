...
, kara(agi::make_unique<AssKaraoke>())
{
	...
	cancel_button = new wxBitmapButton(this, -1, wxNullBitmap);
	accept_button = new wxBitmapButton(this, -1, wxNullBitmap);
	cancel_button->SetBitmap(CMD_ICON_BUNDLE_GET(kara_split_cancel, wxLayout_Default));
	accept_button->SetBitmap(CMD_ICON_BUNDLE_GET(kara_split_accept, wxLayout_Default));
#else
