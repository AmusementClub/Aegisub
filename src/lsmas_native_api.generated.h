/* Auto-generated by vendor/lsmasnative/gen_lsmas_native_api.cs.
 * Source: lsmas_native.h
 * SHA256: 028176a0c18a8024fec5163f125af8a99041b5bfa030917f1cbc9a511fb7c562
 */
#pragma once
...
char *lsmas_probe_streams_json_utf8(
...
int64_t lsmas_video_get_frame_pts(
    lsmas_handle_t *handle,
...
int32_t lsmas_video_get_pts_list(
    lsmas_handle_t *handle,
    int64_t *out_pts,
    int32_t out_count,
    char **error_message
...
int32_t lsmas_video_get_source_frame_count(
    lsmas_handle_t *handle,
    int32_t *out_count,
    char **error_message
);
...
int32_t lsmas_video_get_source_pict_type_list(
    lsmas_handle_t *handle,
    int8_t *out_types,
    int32_t out_count,
    char **error_message
...
int32_t lsmas_video_get_source_keyframe_flags(
    lsmas_handle_t *handle,
    uint8_t *out_flags,
    int32_t out_count,
    char **error_message
...
/* Returns NULL on failure; if error_message != NULL, it will be malloc()'d and must be freed via lsmas_free(). */
lsmas_handle_t *lsmas_video_open_utf8(
    const char *file_path_utf8,
...
lsmas_handle_t *lsmas_video_open_with_progress_utf8(
    const char *file_path_utf8,
    const lsmas_video_open_options_t *options,
...
int lsmas_video_get_stream_props(
    lsmas_handle_t *handle,
...
int lsmas_video_get_stream_mastering_display_metadata(
    lsmas_handle_t *handle,
    lsmas_mastering_display_metadata_t *out_metadata,
    char **error_message
);

int lsmas_video_get_stream_content_light_metadata(
    lsmas_handle_t *handle,
    lsmas_content_light_metadata_t *out_metadata,
    char **error_message
);

int lsmas_video_get_stream_dovi_conf(
    lsmas_handle_t *handle,
...
int lsmas_video_seek_frame(
    lsmas_handle_t *handle,
    int32_t frame_index, /* 0-based */
    char **error_message
);

int lsmas_video_flush(
    lsmas_handle_t *handle,
    char **error_message
);

/* Frame-level metadata (decodes the requested frame). */
int lsmas_video_get_frame_props(
    lsmas_handle_t *handle,
    int32_t frame_index, /* 0-based */
    lsmas_video_props_t *out_props,
    char **error_message
...
int lsmas_video_acquire_avframe(
    lsmas_handle_t *handle,
    int32_t frame_index,      /* 0-based */
...
const void *lsmas_video_frame_get_avframe(
    const lsmas_video_frame_t *frame
);

const char *lsmas_video_frame_get_pix_fmt_name(
    const lsmas_video_frame_t *frame
);

int lsmas_video_frame_get_props(
    const lsmas_video_frame_t *frame,
    lsmas_video_frame_props_t *out_props,
    char **error_message
);

int lsmas_video_frame_get_format_info(
    const lsmas_video_frame_t *frame,
    lsmas_video_format_info_t *out_info,
    char **error_message
);

int lsmas_video_frame_get_plane(
    const lsmas_video_frame_t *frame,
    int32_t plane_index,
...
int lsmas_video_frame_get_side_data(
    const lsmas_video_frame_t *frame,
...
int lsmas_video_frame_get_dovi_metadata(
    const lsmas_video_frame_t *frame,
...
lsmas_handle_t *lsmas_audio_open_utf8(
    const char *file_path_utf8,
 ...
 * Return value from callback: 0 => continue, non-zero => abort. */
lsmas_handle_t *lsmas_audio_open_with_progress_utf8(
    const char *file_path_utf8,
    const lsmas_audio_open_options_t *options,
    lsmas_progress_callback_t progress_cb,
    void *progress_userdata,
...
lsmas_handle_t *lsmas_av_open_utf8(
    const char *file_path_utf8,
 ...
 * Return value from callback: 0 => continue, non-zero => abort. */
lsmas_handle_t *lsmas_av_open_with_progress_utf8(
    const char *file_path_utf8,
    const lsmas_video_open_options_t *video_options,
    const lsmas_audio_open_options_t *audio_options,
    lsmas_progress_callback_t progress_cb,
...
void lsmas_av_close( lsmas_handle_t *handle );

int lsmas_audio_get_info(
    lsmas_handle_t *handle,
 ...
 * Patched HOE builds exclude declared codec padding from the indexed timeline and
 * apply initial padding only when decoding from the stream start. MOV audio
 * endpoints remain valid after tail probes and seeks. The timeline includes
 * the actual resampler EOF output. API version describes interface compatibility;
 * use release/build metadata to identify the implementation and its fixes.
 * Returns the number of sample-frames written (not bytes), or -1 on failure.
 * Requests before/after the output timeline and positive A/V delay padding are
 * filled with silence and count as successful output. An incomplete decode
 * inside the timeline fails the entire request; discard dst and retry the full
 * range if appropriate. error_message is allocated with lsmas_free ownership. */
int64_t lsmas_audio_get_samples(
    lsmas_handle_t *handle,
    void *dst,
