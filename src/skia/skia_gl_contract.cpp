...
char const *ToString(SkiaGlDeviceFailure failure) noexcept {
	switch (failure) {
		...
		case SkiaGlDeviceFailure::SoftwareRendererUnsupported: return "software-renderer-unsupported";
		...
		case SkiaGlDeviceFailure::ContentShaderUnavailable: return "content-shader-unavailable";
		case SkiaGlDeviceFailure::ContentUploadFailed: return "content-upload-failed";
		case SkiaGlDeviceFailure::FlushInjected: return "flush-injected";
		case SkiaGlDeviceFailure::SwapBuffersFailed: return "swap-buffers-failed";
...
bool SkiaGlDeviceState::BeginAccess(SkiaGlContextToken token, std::thread::id current_thread) {
	...
	if (owner.identity != token.identity) {
		Fail(SkiaGlDeviceFailure::ContextIdentityMismatch, "the GL context identity changed without rebuilding the device");
		return false;
	}
	if (owner.generation != token.generation) {
		Fail(SkiaGlDeviceFailure::ContextGenerationMismatch, "the GL context generation changed without rebuilding the device");
		return false;
	}
