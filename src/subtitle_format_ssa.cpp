...
std::string replace_commas(std::string str) {
	agi::util::strings::replace_all_inplace(str, ",", ";");
	return str;
}

std::string strip_newlines(std::string str) {
	agi::util::strings::replace_all_inplace(str, "\n", "");
	agi::util::strings::replace_all_inplace(str, "\r", "");
	return str;
}
}
...
void SsaSubtitleFormat::WriteFile(const AssFile *src, agi::fs::path const& filename, agi::vfr::Framerate const& fps, std::string const& encoding, std::shared_ptr<agi::SingleChoiceInteractionSink> choice_sink) const {
	...
	for (auto const& line : src->Info)
		file.WriteLineToFile(agi::util::strings::iequals(line.Key(), "scripttype") ? "ScriptType: v4.00" : line.GetEntryData());
	...
	for (auto const& line : src->Events) {
		auto const projected = ProjectAssDialogueTimesForStorage(line.Start, line.End, projection_fps);
		file.WriteLineToFile(agi::format("%s: Marked=0,%s,%s,%s,%s,%d,%d,%d,%s,%s"
			, (line.Comment ? "Comment" : "Dialogue")
			, agi::Time(projected.first).GetAssFormatted(), agi::Time(projected.second).GetAssFormatted()
