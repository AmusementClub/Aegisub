...
inline Vector2D ProjectScaledRotatedPoint(Vector2D local, Vector2D origin, Vector2D scale, float rx, float ry, float rz) {
	Vec3 v {
	...
	v = RotateXNeg(v, rx);
	v = RotateYNeg(v, ry);
	return ProjectPerspective(v, origin);
}

inline Vector2D ProjectRotatedShearedPoint(Vec3 v, Vector2D origin, float ax, float ay, float az, float fax, float fay) {
...
inline float ProjectCircleRadius(Vector2D center_local, float radius, Vector2D origin, Vector2D scale, float rx, float ry, float rz) {
	Vector2D const center = ProjectScaledRotatedPoint(center_local, origin, scale, rx, ry, rz);
	Vector2D const px = ProjectScaledRotatedPoint(center_local + Vector2D(radius, 0.0f), origin, scale, rx, ry, rz);
	Vector2D const py = ProjectScaledRotatedPoint(center_local + Vector2D(0.0f, radius), origin, scale, rx, ry, rz);
...
inline void DrawProjectedLine(VideoOverlayDrawContext &context, Vector2D p1, Vector2D p2, Vector2D origin, Vector2D scale, float rx, float ry, float rz) {
	context.DrawLine(
		ProjectScaledRotatedPoint(p1, origin, scale, rx, ry, rz),
		ProjectScaledRotatedPoint(p2, origin, scale, rx, ry, rz));
}

inline void DrawProjectedLine(VideoOverlayDrawContext &context, Vec3 p1, Vec3 p2, Vector2D origin, float ax, float ay, float az, float fax, float fay) {
	context.DrawLine(
		ProjectRotatedShearedPoint(p1, origin, ax, ay, az, fax, fay),
		ProjectRotatedShearedPoint(p2, origin, ax, ay, az, fax, fay));
}

inline void DrawProjectedFadedLine(VideoOverlayDrawContext &context, Vec3 p1, Vec3 p2, Vector2D origin, float ax, float ay, float az, float fax, float fay, wxColour const& colour, float alpha1, float alpha2, int width) {
	...
	for (int i = 0; i < segments; ++i) {
		...
		DrawProjectedLine(context, a, b, origin, ax, ay, az, fax, fay);
	}
}

inline void DrawProjectedQuad(VideoOverlayDrawContext &context, Vector2D p1, Vector2D p2, Vector2D p3, Vector2D p4, Vector2D origin, Vector2D scale, float rx, float ry, float rz) {
	Vector2D const polygon[] = {
		ProjectScaledRotatedPoint(p1, origin, scale, rx, ry, rz),
		ProjectScaledRotatedPoint(p2, origin, scale, rx, ry, rz),
		ProjectScaledRotatedPoint(p3, origin, scale, rx, ry, rz),
		ProjectScaledRotatedPoint(p4, origin, scale, rx, ry, rz),
	};
